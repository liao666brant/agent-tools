#!/usr/bin/env python3
# noqa: SIZE_OK — Upstream self-contained multi-harness collector; splitting complicates skill distribution.
"""采集本地 Claude Code、Codex、Warp、Pi、Grok Build 和 ZCode 会话及技能，用于评分。

扫描 Claude Code 项目历史、Codex rollout 文件、Warp 本地会话数据库、
Pi 会话 JSONL、Grok Build 聊天历史和 ZCode model-io rollout，
发现已安装技能、检测各会话使用的技能，并输出：

  <out>/inventory.json        - 技能、逐会话统计与采样决策
  <out>/transcripts/<id>.md   - 采样会话的精简记录

所有处理均在本机完成，不上传任何内容。仅使用 Python 3.9+ 标准库。
"""
...
def parse_args():
    ...
    p.add_argument(
        "--pi-home",
        default="~/.pi/agent",
        help="Pi Agent 主目录（默认：~/.pi/agent）",
    )
    p.add_argument(
        "--grok-home",
        default="~/.grok",
        help="Grok Build 主目录（默认：~/.grok）",
    )
    p.add_argument(
        "--zcode-home",
        default="~/.zcode",
        help="ZCode 主目录（默认：~/.zcode）",
    )
    p.add_argument(
        "--warp-db",
        action="append",
        default=[],
        help="显式指定 Warp warp.sqlite 路径，可重复传入",
    )
    p.add_argument(
        "--warp-data-dir",
        default=os.environ.get("WARP_DATA_DIR"),
        help="包含 Warp 各渠道数据目录的目录",
    )
    p.add_argument(
        "--repo",
        action="append",
        default=[],
        help="目标项目，可重复传入（默认：当前目录的 Git 根目录，否则为当前目录）",
    )
    p.add_argument(
        "--all-conversations",
        action="store_true",
    ...
    p.add_argument("--include-global-skills", action="store_true",
                   help="同时发现仓库外的技能（~/.codex/skills、~/.agents/skills、~/.claude/skills、~/.pi/agent/skills、~/.grok/skills、~/.zcode/skills）")
    p.add_argument("--days", type=int, default=45, help="只考虑最近 N 天内修改的会话")
    p.add_argument("--max-sessions", type=int, default=12, help="评分会话的最大采样数")
    p.add_argument("--per-skill", type=int, default=3, help="每个技能的最大采样会话数")
    p.add_argument("--no-skill", type=int, default=4, help="未使用技能的最大采样会话数")
    p.add_argument("--skills-dir", action="append", default=[], help="补充要扫描的技能目录，可重复传入")
    p.add_argument("--include-subagents", action="store_true", help="包含子代理或子会话")
...
def extract_text(content) -> str:
    ...
    if isinstance(content, list):
        for block in content:
            if isinstance(block, dict):  # noqa: IF_VARIANT_OK — JSON blocks are an open-ended input shape.
...
def find_warp_conversations(databases, cutoff):
    ...
    for database in databases:
        ...
        try:
            ...
            print(f"警告：无法读取 Warp 数据库 {database}：{exc}", file=sys.stderr)
            continue
        finally:
...
def parse_warp_conversation(record, skill_names, include_subagents):
    ...
    try:
        ...
        print(
            f"警告：无法解码 Warp 会话 "
            f"{record['conversation_id']}（来源：{record['channel']}）：{exc}",
            file=sys.stderr,
        )
        return None
...
def render_transcript(meta, stats, skills_used, entries) -> str:
    lines = [
        f"# 会话 {meta.get('id')}",
        f"- 工作目录：{meta.get('cwd')}",
        f"- 开始时间：{meta.get('started_at') or stats.get('first_ts')}",
        f"- 检测到的技能：{', '.join(skills_used) or '（无）'}",
        f"- 统计：用户 {stats['user_turns']} 轮，助手 {stats['assistant_turns']} 轮，"
        f"工具调用 {stats['tool_calls']} 次（重复 {stats['repeated_tool_calls']} 次），"
        f"疑似错误输出 {stats['error_outputs']} 次，代码编辑：{stats['has_code_edits']}",
        "",
        "## 精简会话记录",
        "",
    ]
    ...
    if len(entries) > MAX_TRANSCRIPT_ENTRIES:
        ...
        shown = entries[:TRANSCRIPT_HEAD] + [("note", f"[... 已省略 {omitted} 条记录 ...]")] + entries[-TRANSCRIPT_TAIL:]
...
def main():
    ...
    if args.all_conversations and args.repo:
        print(
            "错误：--all-conversations 与 --repo 不能同时使用，请选择一种会话范围",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    if requested_claude and (claude_home / "projects").is_dir():
        ...
        print(
            f"错误：未在 {claude_home / 'projects'} 找到 Claude Code 项目历史",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if requested_warp:
        ...
        if warp_databases:
            ...
            print("错误：未找到 Warp 会话数据库", file=sys.stderr)
            sys.exit(1)
    ...
    if requested_pi and (pi_home / "sessions").is_dir():
        ...
        print(f"错误：未在 {pi_home / 'sessions'} 找到 Pi 会话，请检查 --pi-home", file=sys.stderr)
        sys.exit(1)
    ...
    if requested_grok and (grok_home / "sessions").is_dir():
        ...
        print(f"错误：未在 {grok_home / 'sessions'} 找到 Grok Build 会话，请检查 --grok-home", file=sys.stderr)
        sys.exit(1)
    ...
    if requested_zcode and (zcode_home / "cli" / "rollout").is_dir():
        ...
        print(
            f"错误：未在 {zcode_home / 'cli' / 'rollout'} 找到 ZCode model-io rollout，请检查 --zcode-home",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if not sources:
        print(
            "错误：未找到支持的会话来源（Claude Code、Codex、Warp、Pi、Grok Build 或 ZCode），请检查对应的会话目录参数",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    print(f"发现技能：        {st['skills_found']}（时间窗口内使用 {st['skills_used']} 个）")
    print(f"窗口内会话：      {st['session_records_in_window']} 条记录，{st['sessions_in_scope']} 条属于所选范围，{st['sessions_considered']} 条可评分")
    print(f"采样会话：        {st['sessions_sampled']} -> {transcripts_dir}")
    print(f"清单：            {out_dir / 'inventory.json'}")
