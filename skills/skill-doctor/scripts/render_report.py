...
def render_diff(diff_text: str, proposed_path: str = "") -> str:
    ...
    return (
        ...
        '<button class="diff-toggle" type="button" hidden>展开</button>'
        "</div>"
    )
...
def embedded_diffs_script() -> str:
    if not DIFFS_BUNDLE_PATH.exists():
        raise RuntimeError(  # noqa: GENERIC_ERR_OK — Missing vendored asset is a terminal packaging error.
            f"缺少 @pierre/diffs 资源包：{DIFFS_BUNDLE_PATH}；"
...
@media (max-width: 480px) {
  ...
  .title-row h1 { flex-basis: 100%; font-size: 28px; letter-spacing: -1px; }
  .title-row .cta-button { margin-left: auto; }
  .suggestion-change { display: block; margin-top: 4px; }
  .scorecard { flex-direction: column; align-items: stretch; gap: 24px; padding: 22px; }
  .grade-col { width: auto; }
...
def render_page(r) -> str:
    ...
    stat_cells = "".join(
        ...
        for value, label in [
            (stats.get("sessions_analyzed", 0), "已评分会话"),
            (stats.get("skills_found", 0), "已安装技能"),
            (stats.get("skills_used", 0), "已使用技能"),
        ]
    )
    findings = "".join(
        f'<li class="natural-wrap">{esc(finding)}</li>'
        for finding in r.get("top_findings", [])
    )
    suggestions = "".join(
        f"""<li><b><code>{esc(s.get('skill'))}</code></b> <span class="suggestion-change natural-wrap">—{esc(s.get('change'))}</span>
        {('<div class="muted natural-wrap">证据：' + esc(s['evidence']) + '</div>') if s.get('evidence') else ''}
        {render_diff(s.get('diff', ''), s.get('proposed_path', ''))}</li>"""
        for s in r.get("suggestions", [])
    ) or "<li>本时间窗口内没有达到修改门槛的技能建议。</li>"
    ...
    card_data = json.dumps({
        ...
        "grade": grade,
        "grade_label": f"总分 {pct(scores['overall'])}",
        "bars": [
        ...
        "meta": f"发现 {stats.get('sessions_scanned', 0)} 条会话 \u00b7 "
                f"最近 {stats.get('window_days', 45)} 天",
        "stats": [
            [str(stats.get("sessions_analyzed", 0)), "已评分会话"],
...
<div class="stamp-row row factories-footer">
  ...
  <a class="cta-button" href="{esc(r.get('cta_url', 'https://warp.dev/factories/request-access'))}">申请体验</a>
</div>
...
(function () {
  ...
  // --- natural CJK line breaking ----------------------------------------
  function addNaturalBreaks(root) {
    ...
    nodes.forEach(function (node) {
      var segments = Array.from(segmenter.segment(node.data));
      ...
      function flush() {
        ...
        fragment.append(span, document.createElement('wbr'));
        phrase = '';
        hasWord = false;
      }
      segments.forEach(function (part) {
        if (/^\s+$/.test(part.segment)) {
          ...
          if (hasWord && Array.from(phrase + part.segment).length > 8) flush();
          phrase += part.segment;
          hasWord = true;
        } else {
          phrase += part.segment;
  ...
  document.querySelectorAll('.natural-wrap').forEach(addNaturalBreaks);
